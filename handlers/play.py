...
async def play(_, message: Message):
    ...
    if message.chat.id in callsmusic.pytgcalls.active_calls:
        ...
        photo="final.png",
        reply_markup=keyboard,
        caption="✯𝗩𝗲𝗻𝗼𝗺 𝗠𝘂𝘀𝗶𝗰 𝗕𝗼𝘁✯▶️ **Playing** here the song requested by {} 😜".format(
...
async def deezer(client: Client, message_: Message):
    ...
    try:
        ...
        await lel.edit(
            f"<i>Looks like @VENOM_VC Userbot not in this chat, Ask admin to send /play command for first time or add assistant manually</i>"
        )
        return                            
    ...
    if message_.chat.id in callsmusic.pytgcalls.active_calls:
        ...
        appendable = [s_name, r_by, loc]
        qeue.append(appendable)
        await res.edit_text(f"✯𝗩𝗲𝗻𝗼𝗺 𝗠𝘂𝘀𝗶𝗰 𝗕𝗼𝘁✯=#️⃣ Queued at position {position}")
    else:
        await res.edit_text("✯𝗩𝗲𝗻𝗼𝗺 𝗠𝘂𝘀𝗶𝗰 𝗕𝗼𝘁✯=▶️ Playing.....")
...
async def jiosaavn(client: Client, message_: Message):
           ...
           for administrator in administrators:
                      if administrator == message_.from_user.id:  
                          ...
                          try:
                              ...
                              await USER.send_message(message_.chat.id,"I joined this group for playing music in VC")
                              await lel.edit(
                                  "<b>@VENOM_VC helper userbot joined your chat</b>",
                              )
    ...
    try:
        ...
        await lel.edit(
            "<i> @VENOM_VC helper Userbot not in this chat, Ask admin to send /play command for first time or add assistant manually</i>"
        )
        return     
    requested_by = message_.from_user.first_name
    chat_id=message_.chat.id
    ...
    if message_.chat.id in callsmusic.pytgcalls.active_calls:
        ...
        m = await client.send_photo(
            ...
            reply_markup=keyboard,
            photo="final.png",
            caption=f"✯𝗩𝗲𝗻𝗼𝗺 𝗠𝘂𝘀𝗶𝗰 𝗕𝗼𝘁✯=#️⃣ Queued at position {position}",
